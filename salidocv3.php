...
<?
	...
	$mesreal=$mesreal;
	$anioreal=$anioreal;
	$frealen="$dayreal/$mesreal/$anioreal";
...
/* si el campo serie o el campo cantidad esta vacio*/
if($series2 == "EA"){	
    ...
    mssql_query("INSERT INTO salordncv1(idsalidaocv,foliosalocv,idcompraventa,diasalcv,messalcv,aniosalcv,
	             fechsalocv,clave,idtransporte,nomfactura,dirfactura,ciudadfactura,estadofactura,rfcfactura,
				 nofacturaEA,direntrega1,ciudadentrega1,estadoentrega1,entregadaa,recibidapor,comentars)
	             VALUES($code,'$codorden',$idocv,$dia,'$mes',$year,'$fecharecadq','$cod',$trnsprt,'$nom',
				 '$dirfactura','$ciudadfactura','$estadofactura','$rfcfactura','$numerofact','$dirent','$ciudadent',
				 '$estadoent','$entregaa','$recibidox','$comenta')",$conn1);
	...
	$resulta6=mssql_query("SELECT cantentregacv FROM ordencv2 WHERE idcompraventa=$idocv AND cod=$cproc1",$conn1);
    $lafi6=mssql_fetch_array($resulta6);
    $laf6=$lafi6["cantentregacv"];
    $cantisal6=$laf6;			
    $cantisal7=$cantisal6+$canti;
	 /* actualizar la base de datos , las fechas de entrega y transporte - carta porte */
	mssql_query("UPDATE ordencv1 SET idtransporte=$trnsprt,cartaporte='$cpor',fecharealent1='$frealen',fecharealent2='$frealen' WHERE idcompraventa=$idocv",$conn1);
	...
	$resulta3=mssql_query("SELECT cantsalid FROM inventa2 WHERE serial='$serie'",$conn1);
    $lafi3=mssql_fetch_array($resulta3);
    $laf3=$lafi3["cantsalid"];
    $cantisal3=$laf3;			
	$cantisal4=$cantisal3+$canti;					 
	mssql_query("UPDATE inventa2 SET cantsalid=$cantisal4,fechasalida1='$fecharecadq',fechasalida2='$fecharecadq' WHERE serial='$serie'",$conn1);
...
/* si series2 es diferente de EA y la cantidad es igual al numero de series (productos con numero de series) */
	...
	echo "<form name='formu' method='post' action='salidocv2.php'>
		  <input type='hidden' name='idocv' value=$idocv></form>";
...
?>
<? if($entregatotal == 0):
     mssql_query("UPDATE ordencv1 SET statusordencv='Enviada' WHERE idcompraventa=$idocv",$conn);
...
/* selecciono LOS VALORES A LAS TABLAS CORRESPONDIENTES */ 
	...
	<table width="100%"  border="0">
  <tr>
    <td width="49%" align="left" class="main_text_bold">Orden de Compra: <? echo $regis2;?></td>
    <td width="21%">
  ...
  <tr>
    ...
    <td>
	<A class=main_navigation 
    href="foliosalida.php?ursec=<? echo $cod;?>&ndn=<? echo $code;?>&idadq=<? echo $idocv;?>&secur=<? echo session_id();?>" target="_blank">
	Imprimir Recibo
	</A>
	</td>
    <td>&nbsp;</td>
...
<tr> 
 ...
 <td valign="middle" colspan="2"><select name="cproc" style="width:95%; font-size:.85em;">
   <?
