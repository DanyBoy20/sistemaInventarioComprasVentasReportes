...
?>
<?
$refe=strtoupper(trim($ref));
$logoprecio="$ ";
$conn=mssql_connect("SISTEMAS","","");
mssql_select_db("baseDatos");
$pru=mssql_query("SELECT idcompraventa FROM ordencv1 WHERE ncompraventa='$refe'",$conn);
$fi2=mssql_fetch_array($pru);
$uno2=$fi2["idcompraventa"];
$uno3=$fi2["ncompraventa"];
?>
<? if($refe == ""):?>
<?
...
<? elseif($uno2 == ""):?>
<?
echo "<script>alert('NO EXISTE ESA ORDEN !!!');</script>";
echo "<script>window.location.href='salirs.php';</script>";
?>
<? else:?>
<HTML>
...
$result5=mssql_query("SELECT idcompraventa,ncompraventa,diacv,mescv,aniocv,idtransporte,cve_distribuidor,direnviocv,ciudadcv,estadocv,
			            ...
			            FROM ordencv1 WHERE ncompraventa='$refe'",$conn);
$ficha2=mssql_fetch_array($result5);
$uno=$ficha2["idcompraventa"];
